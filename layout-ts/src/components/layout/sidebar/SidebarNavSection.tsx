...
export default function SidebarNavSection({ title, items, isCollapsed, currentPath } : NavProps) {
    return (
            ...
            <div className={`mt-2 space-y-[8px] ${isCollapsed ? 'px-0' : 'px-4'}`}>
                {items.map(({ label, route, icon: Icon }) => (
                    <a
                        ...
                        className={`flex w-full cursor-pointer items-center rounded-md py-1.5 text-left text-[16px] text-[#404040] font-regular no-underline transition-colors ${
                            currentPath === route
