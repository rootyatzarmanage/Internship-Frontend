...
export default function ProjectMap({ pin, onPinChange }) {
  ...
  return (
    <section className="flex h-full min-h-0 w-full flex-col overflow-hidden rounded-xl border border-[#bbbbbb] bg-white p-3 shadow-[0_1px_2px_rgba(0,0,0,0.04)]" aria-labelledby="project-location-title">
      <div className="flex items-center justify-between gap-3"><h2 id="project-location-title" className="text-[20px] font-medium text-[#414141]">Project location</h2><button type="button" aria-pressed={isAdjustingPin} className={`flex items-center gap-1 rounded-lg border px-2.5 py-1.5 text-[10px] transition-colors ${isAdjustingPin ? 'border-[#008cd2] bg-[#008cd2] text-white' : 'border-[#d3d3d3] text-[#555] hover:border-[#008cd2] hover:text-[#008cd2]'}`} onClick={() => setIsAdjustingPin((isAdjusting) => !isAdjusting)}><MapPin size={13} /> {isAdjustingPin ? 'Select location' : 'Adjust pin'}</button></div>
