...
export default function ProjectMap({ pin, onPinChange }) {
  ...
  return (
    <section className="flex h-full min-h-0 w-full flex-col overflow-hidden rounded-xl border border-[#bbbbbb] bg-white p-3 shadow-[0_1px_2px_rgba(0,0,0,0.04)]" aria-labelledby="project-location-title">
      <div className="flex items-center justify-between gap-3"><h2 id="project-location-title" className="text-[20px] font-medium text-[#414141]">Project location</h2><button type="button" aria-pressed={isAdjustingPin} className={`flex items-center gap-1 rounded-lg border px-2.5 py-1.5 text-[10px] transition-colors ${isAdjustingPin ? 'border-[#000000] bg-[#000000] text-white dark:border-white dark:bg-white dark:text-black' : 'border-[#d3d3d3] text-[#555] hover:border-gray-400 hover:text-black dark:border-[#404040] dark:bg-[#151515] dark:text-gray-200 dark:hover:border-gray-300 dark:hover:bg-[#202020] dark:hover:text-white'}`} onClick={() => setIsAdjustingPin((isAdjusting) => !isAdjusting)}><MapPin size={13} /> {isAdjustingPin ? 'Select location' : 'Adjust pin'}</button></div>
